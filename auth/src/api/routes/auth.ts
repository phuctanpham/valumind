...
// Resend Verification
auth.post('/resend-verification', async (c: Context) => {
  try {
...
// Validate Token
auth.get('/validate', async (c: Context) => {
  try {
    const authHeader = c.req.header('Authorization');
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return c.json({ error: 'Unauthorized' }, 401);
...
stepUp.post('/initiate', async (c: Context) => {
    try {
        ...
        const expiresAt = Math.floor(addHours(new Date(), 1/6).getTime() / 1000);
...
stepUp.post('/verify', async (c: Context) => {
    try {
